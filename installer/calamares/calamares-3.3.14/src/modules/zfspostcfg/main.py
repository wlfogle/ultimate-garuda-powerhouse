...
def run():
    ...
    try:
        ...
        # Configure dracut for ZFS (Garuda uses dracut, not mkinitcpio)
        result = configure_dracut(root_mount_point)
        if result:
            return result
...
def configure_dracut(root_mount_point):
    """
    Configure dracut for ZFS support (Garuda Linux uses dracut)
    """
    ...
    try:
        ...
        # Create ZFS configuration for dracut
        zfs_conf_path = os.path.join(dracut_conf_dir, "90-zfs.conf")
        with open(zfs_conf_path, "w") as f:
            f.write("# ZFS configuration for dracut\n")
            f.write("add_dracutmodules+=\"zfs\"\n")
            f.write("install_items+=\"/etc/zfs/zpool.cache\"\n")
            f.write("compress=\"zstd\"\n")
        
        # Also ensure ZFS module is included
        modules_conf_path = os.path.join(dracut_conf_dir, "90-zfs-modules.conf")
        with open(modules_conf_path, "w") as f:
            f.write("# ZFS kernel modules\n")
            f.write("force_drivers+=\"zfs\"\n")
        
        # Regenerate initramfs using dracut
        cmd = ["chroot", root_mount_point, "dracut", "-f", "-v"]
        result = subprocess.run(cmd, check=True, capture_output=True, text=True)
        libcalamares.utils.debug("Regenerated initramfs with dracut")
        
        return None
        
    except Exception as e:
        return f"Failed to configure dracut: {str(e)}"
