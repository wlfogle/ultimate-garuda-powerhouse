...
if ! command -v podman &> /dev/null; then
    ...
    if command -v pacman &> /dev/null; then
        pkexec pacman -S --noconfirm podman podman-compose
    elif command -v apt &> /dev/null; then
        pkexec apt update && pkexec apt install -y podman podman-compose
    else
...
pkexec mkdir -p ${DATA_DIR}/{movies,tv,music,books,downloads,torrents,completed}
pkexec mkdir -p ${CONFIG_DIR}/{radarr,sonarr,lidarr,jackett,jellyfin,qbittorrent}
pkexec chown -R $(whoami):$(whoami) ${DATA_DIR} ${CONFIG_DIR} 2>/dev/null || true
