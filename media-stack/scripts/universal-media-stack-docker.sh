...
# Universal Garuda Media Stack - Docker Version
...
echo -e "${BLUE}🚀 Universal Garuda Media Stack (Docker)${NC}"
echo -e "${BLUE}===========================================${NC}"
...
docker stop ${STACK_NAME}-qbittorrent ${STACK_NAME}-jellyfin ${STACK_NAME}-radarr ${STACK_NAME}-sonarr ${STACK_NAME}-lidarr ${STACK_NAME}-jackett ${STACK_NAME}-jellyseerr 2>/dev/null || true
docker rm ${STACK_NAME}-qbittorrent ${STACK_NAME}-jellyfin ${STACK_NAME}-radarr ${STACK_NAME}-sonarr ${STACK_NAME}-lidarr ${STACK_NAME}-jackett ${STACK_NAME}-jellyseerr 2>/dev/null || true
...
# Check service status and display URLs
echo ""
