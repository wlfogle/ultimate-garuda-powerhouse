...
DATA_DIR="${DATA_DIR:-/media}"
CONFIG_DIR="${CONFIG_DIR:-$USER_HOME/.config}"
...
mkdir -p ${DATA_DIR}/{movies,tv,music,books,downloads,torrents,completed}
mkdir -p ${CONFIG_DIR}/{radarr,sonarr,lidarr,jackett,jellyfin,qbittorrent}
chown -R "$USER_NAME:$USER_NAME" ${DATA_DIR} ${CONFIG_DIR} 2>/dev/null || true
